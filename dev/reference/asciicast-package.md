...
The header is in DCF format (see
[`read.dcf()`](https://rdrr.io/r/base/dcf.html)), but all lines are
...
- `empty_wait`: How long to wait for empty lines in the script file, in
  seconds.
...
- `idle_time_limit`: Time limit for the cast not printing anything, in
  seconds. By default there is no limit.
...
- `timeout`: Idle timeout, in seconds If the R subprocess running the
  recording does not answer within this limit, it is killed and the
  recording stops. Update this for slow running code, that produces no
  output as it runs.

- `timestamp`: Time stamp of the recording, defaults to
...
- `author_img_url`: URL of the author's image, displayed in the titlebar
  in fullscreen mode.
...
- `poster_frame`: Which frame to use (in seconds) as the preview
  picture.
...
- `theme`: Theme to use, currently it has to be a string referring to a
  build-in theme, or a named list of theme properties, see
  ...
  `"github-dark"`, `"pkgdown"` ,`"readme"`. `"readme"` is a special
  theme the switches between light and dark mode automatically in
